...
# This must be the last thing in your script
st.markdown("""
<div id="meshu-debug" style="position: fixed; top: 0; left: 0; background: red; color: white; padding: 5px; z-index: 10000;">MESHU DEBUG</div>
<div id="meshu-chatbot-container" style="position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); z-index: 9999;">
    <button id="meshu-toggle" style="width: 60px; height: 60px; border-radius: 50%; background: linear-gradient(135deg, #2563eb, #7c3aed); border: none; color: white; font-size: 26px; cursor: pointer;">💬</button>
    <div id="meshu-window" style="display: none; position: absolute; bottom: 80px; left: 50%; transform: translateX(-50%); width: 350px; height: 500px; background: white; border: 1px solid black; border-radius: 10px; padding: 10px;">
        <p>Chat window works!</p>
        <button id="meshu-close">Close</button>
    </div>
</div>
<script>
    document.getElementById('meshu-toggle').onclick = function() {
        var win = document.getElementById('meshu-window');
        win.style.display = win.style.display === 'none' ? 'block' : 'none';
    };
    document.getElementById('meshu-close').onclick = function() {
        document.getElementById('meshu-window').style.display = 'none';
    };
</script>
""", unsafe_allow_html=True)
